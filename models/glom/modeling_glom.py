...
from .configuration_glom import GlomConfig
...
class GlomLayerGroup(nn.Module):
    def __init__(self, config):
        ...
        self.albert_layers = nn.ModuleList(
            [GlomLayer(config) for _ in range(config.inner_group_num)]
        )
...
class GlomTransformer(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class GlomPreTrainedModel(PreTrainedModel):
    """
...
@dataclass
class GlomForPreTrainingOutput(ModelOutput):
    """
    Output type of :class:`~transformers.GlomForPreTraining`.
...
ALBERT_START_DOCSTRING = r"""
    ...
    Args:
        config (:class:`~transformers.GlomConfig`): Model configuration class with all the parameters of the model.
...
ALBERT_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`({0})`):
            ...
            Indices can be obtained using :class:`~transformers.GlomTokenizer`. See
...
class GlomModel(GlomPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.config = config
        self.embeddings = GlomEmbeddings(config)
        self.encoder = GlomTransformer(config)
...
class GlomForPreTraining(GlomPreTrainedModel):
    def __init__(self, config):
        ...
        self.albert = GlomModel(config)
        self.predictions = GlomMLMHead(config)
        self.sop_classifier = GlomSOPHead(config)
    ...
    def forward(
        ...
        Example::
            ...
            >>> from transformers import GlomTokenizer, GlomForPreTraining
            >>> import torch

            >>> tokenizer = GlomTokenizer.from_pretrained('albert-base-v2')
            >>> model = GlomForPreTraining.from_pretrained('albert-base-v2')
        ...
        return GlomForPreTrainingOutput(
            loss=total_loss,
...
class GlomForMaskedLM(GlomPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.glom = GlomModel(config, add_pooling_layer=False)
        self.predictions = GlomMLMHead(config)
